function plotAIMsReaching_controlLimb(controlReachAvgAIMs)

% Plots bar graph of reaching vs non-reaching AIMs scores in control rats 

    % Averages the control group's global AIMs scores
    avgReachData = nanmean(controlReachAvgAIMs.limb(:,1));
    avgNonReachData = nanmean(controlReachAvgAIMs.limb(:,2));

    % Reformats the data for individual plots 
    controlReachData = NaN(2,4);
    controlReachData(1,:) = controlReachAvgAIMs.limb(2,:);
    controlReachData(2,:) = controlReachAvgAIMs.limb(1,:);
    ...
    stdReachData(:,1) = nanstd(controlReachAvgAIMs.limb(:,1))./sqrt(5);
    stdReachData(:,2) = nanstd(controlReachAvgAIMs.limb(:,2))./sqrt(5);
