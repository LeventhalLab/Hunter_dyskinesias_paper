function plotAIMsReaching_controlAxial(controlReachAvgAIMs)

% Plots bar graph of reaching vs non-reaching AIMs scores in control rats 

    % Averages the control group's axial AIMs scores
    avgReachData = nanmean(controlReachAvgAIMs.axial(:,1));
    avgNonReachData = nanmean(controlReachAvgAIMs.axial(:,2));

    % Reformats the data for individual plots 
    controlReachData = NaN(2,4);
    controlReachData(1,:) = controlReachAvgAIMs.axial(2,:);
    controlReachData(2,:) = controlReachAvgAIMs.axial(1,:);
    ...
    ylabel('Day 10 Axial Aims Score')
    ...
    stdReachData(:,1) = nanstd(controlReachAvgAIMs.axial(:,1))./sqrt(5);
    stdReachData(:,2) = nanstd(controlReachAvgAIMs.axial(:,2))./sqrt(5);
