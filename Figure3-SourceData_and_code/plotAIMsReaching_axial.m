function plotAIMsReaching_axial(duringReachAvgAIMs)

% Plots bar graph of reaching vs non-reaching AIMs scores in during rats 

    % Averages the during group's global AIMs scores
     avgReachData = nanmean(duringReachAvgAIMs.axial(:,1));
     avgNonReachData = nanmean(duringReachAvgAIMs.axial(:,2));

     % Reformats the data for individual plots 
     duringReachData = NaN(2,5);
     duringReachData (1,:) = duringReachAvgAIMs.axial(2,:);
     duringReachData (2,:) = duringReachAvgAIMs.axial(1,:);
    ...
    stdReachData(:,1) = nanstd(duringReachAvgAIMs.axial(:,1))./sqrt(5);
    stdReachData(:,2) = nanstd(duringReachAvgAIMs.axial(:,2))./sqrt(5);
