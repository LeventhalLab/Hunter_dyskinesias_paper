function plotAIMsReaching_global(duringReachAvgAIMs)

% Plots bar graph of reaching vs non-reaching AIMs scores in during rats 

    % Averages the during group's global AIMs scores
     avgReachData = nanmean(duringReachAvgAIMs.global(:,1));
     avgNonReachData = nanmean(duringReachAvgAIMs.global(:,2));

     % Reformats the data for individual plots 
     duringReachData = NaN(2,5);
     duringReachData (1,:) = duringReachAvgAIMs.global(2,:);
     duringReachData (2,:) = duringReachAvgAIMs.global(1,:);
    ...
    stdReachData(:,1) = nanstd(duringReachAvgAIMs.global(:,1))./sqrt(5);
    stdReachData(:,2) = nanstd(duringReachAvgAIMs.global(:,2))./sqrt(5);
