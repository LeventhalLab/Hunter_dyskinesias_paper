function plotCylData_during_limb(duringData_limb)

  % plots bar graph of day 10 in the reaching chamber vs day 10 post-reaching cylinder
  % limb AIMs for primary during rats 
   
    avgDuringData = NaN(1,1);
    avgDuringData(1,:) = nanmean(duringData_limb(:,1));
    avgDuringData(2,:) = nanmean(duringData_limb(:,2));
    
    % plots reachimg chamber 
...
stdDuringData(:,1) = nanstd(duringData_limb(:,1))./sqrt(5);
stdDuringData(:,2) = nanstd(duringData_limb(:,2))./sqrt(5);
...
for i_context = 1:2
    xVals = [1, 2];
    for i_rat = 1:5
        plot(xVals,duringData_limb(i_rat,:),'Color',indivColor,'LineWidth',1)
    end
end 
