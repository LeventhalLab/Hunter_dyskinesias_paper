function plotCylData_control_axial(controlData_axial)
 

  % plots bar graph of day 10 in the reaching chamber vs day 10 post-reaching cylinder
  % axial AIMs for control rats 
  
    avgControlData = NaN(1,1);
    avgControlData(1,:) = nanmean(controlData_axial(:,1));
    avgControlData(2,:) = nanmean(controlData_axial(:,2));
...
stdControlData(:,1) = nanstd(controlData_axial(:,1))./sqrt(3);
stdControlData(:,2) = nanstd(controlData_axial(:,2))./sqrt(3);
...
for i_context = 1:2
    xVals = [1, 2];
    for i_rat = 1:3
        plot(xVals,controlData_axial(i_rat,:),'Color',indivColor,'LineWidth',1)
    end
end 
