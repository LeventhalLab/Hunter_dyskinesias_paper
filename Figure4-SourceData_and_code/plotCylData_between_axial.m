...
function plotCylData_between_axial(betweenData_axial)

    % plots bar graph of day 10 in the reaching chamber vs day 10 post-reaching cylinder
    % axial AIMs for between rats 

    avgBetweenData = NaN(1,1);
    avgBetweenData(1,:) = nanmean(betweenData_axial(:,1));
    avgBetweenData(2,:) = nanmean(betweenData_axial(:,2));
...
stdBetweenData(:,1) = nanstd(betweenData_axial(:,1))./sqrt(6);
stdBetweenData(:,2) = nanstd(betweenData_axial(:,2))./sqrt(6);
...
for i_context = 1:2
    xVals = [1, 2];
    for i_rat = 1:6
        plot(xVals,betweenData_axial(i_rat,:),'Color',indivColor,'LineWidth',1)
    end
end 
